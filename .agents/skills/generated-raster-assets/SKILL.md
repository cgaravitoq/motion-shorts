...
A generated image must be referenced from a scene-type slot that takes an image. As of now the 11 scene-types (`hook`, `title-cards`, `flow`, `metric`, `big-stat`, `comparison`, `timeline`, `quote`, `code`, `social-card`, `outro`) expose only `text`, `richText`, and `repeat` slots — **none currently exposes an image slot**.
