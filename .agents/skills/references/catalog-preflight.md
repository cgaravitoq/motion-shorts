...
3. Pick a scene skeleton from the returned `skeleton`, adapting counts/types to the script's beats. **`hook` is always first; `outro` is always last** (the pinned brand sign-off — never a plain `@handle` card). The 13 scene-types are the only building blocks: `hook, title-cards, flow, fanout, metric, bars, big-stat, comparison, timeline, quote, code, social-card, outro`.
