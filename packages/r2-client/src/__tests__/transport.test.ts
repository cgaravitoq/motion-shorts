...
describe("R2 transport", () => {
  it("detects complete gateway or direct-S3 configuration", () => {
    ...
    expect(
      isR2Configured({ bucket: "bucket", env: { R2_ACCOUNT_ID: "account", R2_BUCKET: "bucket" } }),
    ).toBe(false);
  });
  ...
  it("returns null for gateway 404 reads", async () => {
    ...
    await expect(
      getObject({ bucket: "bucket", key: "missing", env: gatewayEnv, fetchImpl }),
    ).resolves.toBeNull();
    await expect(
      headObject({ bucket: "bucket", key: "missing", env: gatewayEnv, fetchImpl }),
    ).resolves.toBeNull();
  });
