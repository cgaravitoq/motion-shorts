...
const resolveConfig = (bucket: string, runtimeEnv: R2ClientEnv = defaultEnv): R2Config | null => {
  if (!runtimeEnv.R2_ACCOUNT_ID || !runtimeEnv.R2_BUCKET || runtimeEnv.R2_BUCKET !== bucket) return null;
...
const hasGatewayTransport = (config: R2Config): boolean => Boolean(config.uploadGatewayUrl && config.uploadGatewayToken);
const hasDirectTransport = (config: R2Config): config is R2Config & { accessKeyId: string; secretAccessKey: string } =>
...
const fetchWithTimeout = (fetchImpl: FetchImpl, url: string, init: RequestInit, timeoutMs: number) => {
  if (fetchImpl !== fetch || !Number.isFinite(timeoutMs) || timeoutMs <= 0) return fetchImpl(url, init);
...
const toBuffer = async (response: Response): Promise<Buffer> => Buffer.from(await response.arrayBuffer());
...
export const putObject = async ({
  ...
  if (hasGatewayTransport(config)) {
    ...
    if (!response.ok) throw new Error(`R2 gateway PUT failed for ${key}: ${response.status} ${response.statusText}`);
    return;
  }
  ...
  const response = await fetchWithTimeout(fetchImpl, signed.url, { method: "PUT", headers: signed.headers, body: bytes }, config.requestTimeoutMs);
  if (!response.ok) throw new Error(`R2 PUT failed for ${key}: ${response.status} ${response.statusText}`);
};

export const getObject = async ({ bucket, key, fetchImpl = fetch, env }: ObjectArgs): Promise<GetObjectResult | null> => {
  const config = requireConfig(bucket, env);
  const response = hasGatewayTransport(config)
    ? await fetchWithTimeout(fetchImpl, gatewayObjectUrl(config, key), { method: "GET", headers: { "x-upload-token": config.uploadGatewayToken ?? "" } }, config.requestTimeoutMs)
    : await (async () => {
        if (!hasDirectTransport(config)) throw new Error("R2 direct-S3 transport is not configured.");
        const signed = signRequest({
          config: { ...config, accessKeyId: config.accessKeyId, secretAccessKey: config.secretAccessKey },
        ...
        return fetchWithTimeout(fetchImpl, signed.url, { method: "GET", headers: signed.headers }, config.requestTimeoutMs);
      })();
...
export const headObject = async ({ bucket, key, fetchImpl = fetch, env }: ObjectArgs): Promise<HeadObjectResult | null> => {
  const config = requireConfig(bucket, env);
  const response = hasGatewayTransport(config)
    ? await fetchWithTimeout(fetchImpl, gatewayObjectUrl(config, key), { method: "HEAD", headers: { "x-upload-token": config.uploadGatewayToken ?? "" } }, config.requestTimeoutMs)
    : await (async () => {
        if (!hasDirectTransport(config)) throw new Error("R2 direct-S3 transport is not configured.");
        const signed = signRequest({
          config: { ...config, accessKeyId: config.accessKeyId, secretAccessKey: config.secretAccessKey },
        ...
        return fetchWithTimeout(fetchImpl, signed.url, { method: "HEAD", headers: signed.headers }, config.requestTimeoutMs);
      })();
  if (response.status === 404) return null;
