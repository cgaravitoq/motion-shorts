...
1. **Monolithic single-file (generated).** Each `apps/hyperframe/src/episodes/<slug>/index.html` contains ALL CSS + HTML + GSAP + captions JSON inline and is **generated by the assembler** from `scene-spec.json` — never hand-edited. Same rule for the optional 16:9 `index.desktop.html` (`assemble <slug> --format=desktop`, see `docs/formats.md`). Zero `data-composition-src` — the runtime force-applies `position: absolute; top:0; left:0; 100%x100%` on tracked stage children, collapsing sub-comp flex layouts.
...
5. **Render.** `bun run render:episode <slug> --format=mp4` once all scenes pass (desktop: add `--variant=desktop-1080p`).
