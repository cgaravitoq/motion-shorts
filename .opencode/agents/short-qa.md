...
2. Run per-scene QA: `bun run scripts/scene-qa.mjs <slug>`. This re-assembles, captures one settled "final" frame per scene plus a `contact-sheet.jpg` grid (`--frames=3` for motion debugging), and runs `hyperframes inspect` for overflow/overlap. No mp4 is rendered.
