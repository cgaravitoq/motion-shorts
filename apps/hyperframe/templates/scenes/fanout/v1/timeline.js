...
function build_fanout(tl, t, s, p) {
  ...
  const outOrigin = isDesktop ? "0% 50%" : "50% 0%";
  const inOrigin = isDesktop ? "100% 50%" : "50% 100%";
  const hide = isDesktop ? { scaleX: 0 } : { scaleY: 0 };
  const draw = isDesktop ? { scaleX: 1 } : { scaleY: 1 };

  tl.set(s(".fo-link--out"), { ...hide, autoAlpha: 0, transformOrigin: outOrigin }, 0);
  tl.set(s(".fo-link--in"), { ...hide, autoAlpha: 0, transformOrigin: inOrigin }, 0);
  ...
  tl.to(s(".fo-link--out"), { ...draw, autoAlpha: 1, duration: 0.34, stagger: 0.08, ease: "power2.out" }, t + 0.7);
  ...
  tl.to(s(".fo-link--in"), { ...draw, autoAlpha: 1, duration: 0.34, stagger: 0.08, ease: "power2.out" }, t + 1.6);
